...
$(function(){
	...
	$('#editor').submit(function(e){
		...
		var words = response.currentLines;
		$('#editor input:text').each(function(){
			var indexes = $(this).data('lineindex').split('_');
			if(words[indexes[0]] == undefined){
				words[indexes[0]] = [];
			}
			words[indexes[0]].words[indexes[1]].string = $(this).val();
		});
		response.currentLines = words;
		$.ajax({
			type: 'POST',
...
function lyrics_main(){
	...
	lnum=0
	for  (var lindex in response.originalLines){
		var lelement = response.originalLines[lindex];
	...
	for  (var lindex in response.currentLines){
		...
		for  (var windex in lelement.words){
			var welement = lelement.words[windex]
			var w = $("<input type='text' id='wE"+welement.start+"' value='"+welement.string+"' data-lineindex='"+lnum+"_"+windex+"'/>");
			lines.append(w)
		}
		$('#editor').append(lines);
		lnum++;
